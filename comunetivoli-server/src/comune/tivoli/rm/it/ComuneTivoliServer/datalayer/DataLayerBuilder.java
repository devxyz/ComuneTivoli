...
public class DataLayerBuilder {
    ...
    public static long maxToken() {
        long max = 0;
        final List<GAE_NotiziaSitoDB_V1> gae_circolareDB_v2s = getLoaderNewsSito().allEntities();
        for (GAE_NotiziaSitoDB_V1 x : gae_circolareDB_v2s) {
            max = Math.max(max, x.getToken());
        }

        final List<GAE_NotiziaFacebook_V0_1> gae_newsDB_v2s = getLoaderNewsFacebook().allEntities();
        for (GAE_NotiziaFacebook_V0_1 x : gae_newsDB_v2s) {
            max = Math.max(max, x.getToken());
        }
        return max;
    }
